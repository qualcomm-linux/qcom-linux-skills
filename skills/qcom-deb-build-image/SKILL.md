...
- To validate a flashed board reaches a login shell over serial, use
  ...
  `root` credentials). The first login forces a password change
  (`chage --lastday 0`), so the console driver must answer the
  `Current password:` / `New password:` / `Retype new password:` prompts —
  see `qcom-deb-flash-boot` for the exact flow.
