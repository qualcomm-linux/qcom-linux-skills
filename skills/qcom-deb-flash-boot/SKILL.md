...
use the new password. An agent driving the console must expect and answer the
`Current password:` / `New password:` / `Retype new password:` prompts — a
plain `debian`/`debian` login without them will not reach a shell.
...
(`--username debian --password debian`). Because the first boot forces the
password change above, either complete that change once manually
(interactively or over the console) before running the plain validation, or
handle the change flow when driving the login — a single `--password debian`
login will otherwise stall at `Current password:`.
